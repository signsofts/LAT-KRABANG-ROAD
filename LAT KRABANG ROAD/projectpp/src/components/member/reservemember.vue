<template>
  ...
  <div class="room">
    ...
    >
      ...
      <p>ราคา : {{ roomDetails.Price }} บาท / ต่อคืน</p>
      ...
      <div class="total-price-container">
        <p v-if="checkOutDate" class="total-price">ราคารวม: {{ getTotalPrice() }} บาท</p>
      </div>
...
export default {
  ...
  methods: {
    checkoutButtonn() {
      ...
      this.thisFrom = document.querySelector("#checkoutForm");
      OmiseCard.configure({
        publicKey: "pkey_test_5v5zy2me9pbw85r9h6i",
        currency: "THB",
        frameLabel: "ชำระเงินผ่านบัตร",
        submitLabel: "จองตอนนี้",
      ...
      OmiseCard.open({
        ...
        onCreateTokenSuccess: (nonce) => {
          if (nonce.startsWith("tokn_")) {
            this.thisFrom.omiseToken.value = nonce;
          } else {
            this.thisFrom.omiseSource.value = nonce;
          }
          // form.submit();
          this.submitForm();
        },
    ...
    submitForm() {
      ...
      const formData = new FormData(this.thisFrom);
      ...
      const reservationData = {
        ID_Room: this.roomDetails.ID_Room,
        ...
        ID_CUS: localStorage.getItem("id_cus"),
        formProps: formProps,
      };
      ...
      axios
        .post("http://localhost:3000/reserve", reservationData)
        .then(async (response) => {
          // console.log(response);
          ...
          axios
            .post("http://localhost:3000/updateRoomStatus", updateRoomStatus)
            .then((updateResponse) => {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              // บันทึก ID_CUS ใน LocalStorage
              localStorage.setItem("id_cus", response.data.id_cus);
